...
- Необходимо нажать **Proceed to 192.168.1.10 (unsafe)**
...
<p align="center"> Рисунок 6 - Задание настроек подсети </p>
...
<p align="center"> Рисунок 7 - Поиск контроллера </p>
...
<p align="center"> Рисунок 10 - Контролеры в проекте </p>
...
<p align="center"> Рисунок 11 - Обзор контроллера </p>

Далее нажимаем на кнопку подключения:

![добавление контроллера](images/PLC_connecting_button.png)
<p align="center"> Рисунок 12 - Подключение к контроллеру </p>
...
<p align="center"> Рисунок 13 - Подключенный контроллер </p>

## Задание пароля

Нажимаем на кнопку изменения пароля и задаем новый пароль для пользователя **admin** - **обратитесь к администратору**:

![изменение пароля](images/change_pass.png)
<p align="center"> Рисунок 14 - Изменение пароля</p>
...
- Переходим на вкладку **Security** -> **User Authentication** и нажимаем на кнопку **Set Password** и задаем новый пароль для пользователя **admin** - **обратитесь к администратору**.
...
- Копировать файл с заменой файла библиотеки из **PtusaPLCnextEngineer.pcwlx\PROJECT\Logical%20Elements\AXCF2152_19.9.0\libPtusaPLCnextEngineer.so** в каталог **/opt/plcnext/projects/PCWE/Libs/Ptusa**. 
...
- Перезапустите службу **plcnext** (команда: **sudo /etc/init.d/plcnext restart restart**).
...
- Переходим на вкладку с настройками контроллера (рисунок 11).

- Далее нажимаем на кнопку подключения (рисунок 12).
...
Далее после ввода пароля текущего пользователя задаем пароль для **root**:
...
- Откройте файл */etc/ssh/sshd_config* в подходящем редакторе.

- В разделе #Authentication: раскомментируйте закомментированную запись. #PermitRootLogin yes.
...
- В разделе #Authentication: закомментируйте раскомментированную запись. PermitRootLogin yes.
...
далее вводим пароль. После успешного переключения отображется соответствующее приглашение командной строки:
...
```sh
tail -F /var/log/messages | grep ptusa
```

## Скрипт автозагрузки

Для автозагрузки надо переключится под пользователя **root**:
...
Затем прописываем скрипт для автозагрузки:
...
HOME = /
#run-parts
@reboot sleep 20 && sudo ifconfig eth0 xxx.xxx.xxx.xxx && sudo ifconfig eth0 yyy.yyy.yyy.yyy netmask iii.iii.iii.iii && sudo route add default gw jjj.jjj.jjj.jjj
```

где:

*xxx.xxx.xxx.xxx* - IP-адрес "пустой", который не используется.  
*yyy.yyy.yyy.yyy* - IP-адрес контроллера.  
*iii.iii.iii.iii* - Маска контроллера.  
*jjj.jjj.jjj.jjj* - Gateway контроллера.  

## Запуск проекта

Перед запуском проекта подключаемся к контроллеру по **ssh** (используя **Kitty**) и запускаем отображение логов:
...
tail -F /var/log/messages /var/log/error | awk '
  ...
  /WARN/ && /ptusa/ {print "\033[33m" $0 "\033[39m"; next}
  /DEBUG/ && /ptusa/ {print "\033[37m" $0 "\033[39m"; next}
  /ptusa/ {print current_color $0 "\033[39m"}
  '
```
...
При успешном запуске в окне просмотра логов не должно быть аварийных сообщений (только информационные - версии управляющей программы, скриптов и т.п.):


![запуск](images/logs.png)
<p align="center"> Рисунок 17 - Просмотр сообщений </p>

В данном случае есть ошибки: нет связи с сервером и присутствует ошибка записи аналоговых данных (возможно, некорректно описаны исполнительные механизмы). Необходимо поправить описание программы (скрипты из **EPLANer**-а), записать новое описание и, используя новое окно консоли **Kitty**, перезапустить проект. В открытом ранее окне консоли **Kitty** отобразятся новые сообщения соответственно ходу перезагрузки проекта.
...
<p align="center"> Рисунок 18 - Утилита IP Assignment</p>
...
<p align="center"> Рисунок 19 - Настройка сетевых параметров</p>
...
<p align="center"> Рисунок 20 - Типовая конфигурация шины <i>Axioline F</i></p>

Открываем данный проект, переходим на вкладку *Axioline F*:

![настройка IOL8](images/PLCnext_AXL_F_IOL8_props.png)
<p align="center"> Рисунок 21 - Настройка IOL8</p>
...
<p align="center"> Рисунок 22 - Запись настроек шины <i>Axioline F</i></p>
...
<p align="center" height="400"> Рисунок 23 - Индикаторы <i>Axioline F</i></p>
...
<p align="center"> Рисунок 24 - Успешное подключение к AXL F BK ETH </p>

Переходим на вкладку *"Startup Behaviour"* и устанавливаем флажок *"Run in plug and play mode after next restart"*. В поле *Enter Password* вводим пароль *private*:

![подключение к AXL F BK ETH](images/AXL_F_BK_ETH_web_pnp_mode.png)
<p align="center"> Рисунок 25 - Успешное подключение к AXL F BK ETH </p>
...
<p align="center"> Рисунок 26 Конфигурация шины Axioline F</p>
